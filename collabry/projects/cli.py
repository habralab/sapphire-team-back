...
def run(ctx: typer.Context):
    ...
    projects_service = get_service(loop=loop, settings=settings)
...
def callback(ctx: typer.Context):
    ...
    if settings := ctx.obj.get("settings"):
        ctx.obj["settings"] = settings.projects
    else:
        ctx.obj["settings"] = get_settings(Settings, env_prefix="PROJECTS__")
